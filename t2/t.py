...
def read_arguments():
    ...
    description = 't is a program for translation words from english to russian'
...
def show_result(word, result):
    # prints out '$word_to_translate -> $result_of_translation
    print('\n>>> %s -> %s \n' % (str(word),str(result)))
...
class inf:
    ...
    def __init__(self, word, lang=None, dic=None):
        ...
        if lang is None: # 'lang' is an argument that controls what languages from and to a word should be translated
            if ord(word[0].lower()) in range(97,122):  # translation from english to russian is a default statement for 'lang' argument
                self.lang = 'en-ru'
            elif ord(word[0].lower()) in range(1072,1104):
                self.lang = 'ru-en'
        else:
            self.lang = lang
        if dic is None:
